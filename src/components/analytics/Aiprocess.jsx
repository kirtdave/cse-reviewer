...
// Smart Adaptive Engine (moved to top-left position)
  ...
  return (
    ...
    >
      ...
      <div className="space-y-4">
        <div className={`flex items-center gap-3 p-4 rounded-xl ${isDark ? "bg-blue-500/10" : "bg-blue-50"} border ${isDark ? "border-blue-500/20" : "border-blue-200"}`}>
        ...
        <div className={`flex items-center gap-3 p-4 rounded-xl ${isDark ? "bg-purple-500/10" : "bg-purple-50"} border ${isDark ? "border-purple-500/20" : "border-purple-200"}`}>
        ...
        <div className={`flex items-center gap-3 p-4 rounded-xl ${isDark ? "bg-green-500/10" : "bg-green-50"} border ${isDark ? "border-green-500/20" : "border-green-200"}`}>
...
const ExamReadinessForecast = ({ theme, data }) => {
  ...
  // Calculate pass probability based on recent performance
  ...
  return (
    <motion.div
      initial={{ opacity: 0, x: 20 }}
      animate={{ opacity: 1, x: 0 }}
      className={`${isDark ? "bg-gradient-to-br from-purple-900/40 to-pink-900/40" : "bg-gradient-to-br from-purple-50 to-pink-50"} backdrop-blur-xl p-6 rounded-2xl border ${isDark ? "border-purple-500/30" : "border-purple-200"} shadow-xl`}
    >
      <div className="flex items-center gap-3 mb-6">
...
// AI Learning Curve Component with Real Data
const AILearningCurve = ({ theme, data }) => {
  ...
  const learningData = data?.trend?.length > 0 
    ? [
        // ✅ CRITICAL FIX: Reverse the array first, THEN map and re-label
        ...[...data.trend].reverse().map((item, index) => ({
          exam: `Mock ${index + 1}`, // ✅ FIXED: Mock 1, Mock 2, etc. in correct order
          confidence: item.score,
          mastery: Math.max(0, item.score - 5) // Mastery slightly lower than score
        })),
        // Add projection if we have enough data
        ...(data.trend.length >= 3 ? [{
          exam: "Projected",
          confidence: Math.min(100, data.trend[0].score + 5), // Use most recent (first in original array)
...
// Strengths & Weaknesses (moved from AnalyticsCharts)
const StrengthsWeaknesses = ({ theme, data }) => {
  const isDark = theme === "dark";

  // ✅ Use real strengths/weaknesses data with ALL 7 CATEGORIES AS FALLBACK
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-green-500 to-emerald-600 flex items-center justify-center shadow-lg">
            <i className="fa-solid fa-chart-simple text-white"></i>
          </div>
          <div>
            <h3 className={`text-lg font-bold ${isDark ? "text-white" : "text-gray-900"}`}>
        ...
        >
          <Activity className={`w-5 h-5 ${isDark ? "text-green-500" : "text-green-600"}`} />
        </motion.div>
      </div>

      {/* ✅ SCROLLABLE CONTAINER - Max 4 items visible, rest scrollable */}
      <div className="max-h-[240px] overflow-y-auto pr-2 space-y-5 scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-transparent">
        {strengthsData.map((sw, i) => (
          ...
          >
            <div className="flex justify-between items-center mb-2">
              ...
              <span className={`text-sm font-bold bg-gradient-to-r ${sw.gradient} bg-clip-text text-transparent`}>
                {sw.value}%
              </span>
            </div>
            <div className={`h-3 rounded-full ${isDark ? "bg-gray-700" : "bg-gray-200"} overflow-hidden`}>
              <motion.div
                ...
                className={`h-3 rounded-full bg-gradient-to-r ${sw.gradient} relative overflow-hidden`}
              >
                {/* Shimmer effect */}
                <motion.div
      ...
      {/* ✅ SCROLL INDICATOR (shows when there are more than 4 items) */}
...
// AI Persona Assistant with Dynamic Messages
const AIPersonaAssistant = ({ theme, data }) => {
  const isDark = theme === "dark";
  const [message, setMessage] = useState(0);

  // Generate personalized messages based on real data
  const getPersonalizedMessages = () => {
    const msgs = [];

    // Check sections for improvement
    ...
    // Default messages if not enough data
    if (msgs.length === 0) {
  ...
  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      className={`${isDark ? "bg-gradient-to-br from-gray-900/80 to-blue-900/20" : "bg-gradient-to-br from-white/80 to-blue-50/80"} backdrop-blur-xl p-6 rounded-2xl border ${isDark ? "border-blue-500/30" : "border-blue-200"} shadow-xl`}
    >
      ...
      <div className={`mt-4 pt-4 border-t ${isDark ? "border-gray-700" : "border-gray-200"}`}>
        <p className={`text-xs ${isDark ? "text-gray-500" : "text-gray-600"} flex items-center gap-2`}>
